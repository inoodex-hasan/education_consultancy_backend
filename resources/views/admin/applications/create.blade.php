                        ...
                        <select name="student_id" id="student_id" class="form-select" required>
                            ...
                            @foreach ($students as $student)
                                <option value="{{ $student->id }}"
                                    {{ old('student_id') == $student->id || $selected_student == $student->id ? 'selected' : '' }}>
                                    {{ $student->first_name }} {{ $student->last_name }} - ({{ $student->phone }})
                                    {{ $student->passport_number ? '- ' . $student->passport_number : '' }}
                                </option>
                            @endforeach
                        </select>
                        @error('student_id')
            ...
            studentSelect.addEventListener('change', function() {
                ...
                safeFetch(`{{ route('admin.applications.get-student-details') }}?student_id=${studentId}`)
                    .then(data => {
                        ...
                        return safeFetch(
                                `{{ route('admin.applications.get-universities') }}?country_id=${data.country_id}`
                                )
                            .then(universities => {
                                ...
                                return safeFetch(
                                        `{{ route('admin.applications.get-courses') }}?university_id=${data.university_id}`
                                        )
                                    .then(courses => {
                                        ...
                                        return safeFetch(
                                                `{{ route('admin.applications.get-intakes') }}?course_id=${data.course_id}`
                                                )
                                            .then(intakes => {

                                                clearDropdown(intakeSelect,
                                                'Select Intake');
                                                ...
                                                intakes.forEach(i => {
                                                    ...
                                                    option.textContent = i
                                                        .intake_name;
                                                    intakeSelect.appendChild(
                                                    option);
                                                });

                                                if (data.course_intake_id) {
                                                    intakeSelect.value = data
                                                        .course_intake_id;
                                                }
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const niceSelect = NiceSelect.bind(studentSelect, {
                searchable: true,
                placeholder: 'Select Student (Search by Name, Phone, Email or Passport)'
            });
                                        ...
                                        if (data.university_id) {
                                            ...
                                            fetch(
                                                ...
                                                .then(courses => {
                                                    ...
                                                    if (data.course_id) {
                                                        courseSelect.value = data.course_id;
                                                        const selectedOption = courseSelect
